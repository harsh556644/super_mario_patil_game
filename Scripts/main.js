...
// AUDIO SETUP
var bgMusic = new Audio('Content/audio/bg.mp3');
bgMusic.loop = true;

var gameOverMusic = new Audio('Content/audio/gameover.mp3');
...
var Level = Base.extend({
    ...
    for(var i = this.figures.length; i--; ) {
        if(this.figures[i] instanceof Mario) {
            ...
            // 🎵 AFTER 3 DEATHS → GAME OVER SOUND
            if(deathCount >= 3) {
                bgMusic.pause();
                gameOverMusic.play();
                setTimeout(function() {
                    if (typeof showEndOverlay === 'function') showEndOverlay('lose');
                }, 1000);
            }
    ...
    if(settings.lifes < 0) {
        bgMusic.pause(); // stop bg music on full game over
        ...
        setTimeout(function() {
            self.start();
        }, 1000); // pause for 1 second before restarting after death
    }
},
	load: function(level) {
		if(this.active) {
	...
	start: function() {
		...
		// ▶️ PLAY BACKGROUND MUSIC ONLY IF NOT ALREADY PLAYING
		if(bgMusic.paused) {
			bgMusic.play();
		}
...
var Mario = Hero.extend({
	...
	victory: function() {
		...
		var levelNumber = this.level.id + 1;
		var nextLevelNumber = levelNumber + 1;
		var videoSrc = levelNumber === 1 ? VIDEOPATH + 'videoforend.mp4' : VIDEOPATH + 'L' + levelNumber + '.mp4';

		if (this.level.id < definedLevels.length - 1) {
			setTimeout(function() {
				if (typeof showEndOverlay === 'function') showEndOverlay('level1', {
					title: 'Level ' + levelNumber + ' complete! Watch the video and start Level ' + nextLevelNumber + '.',
					buttonText: 'Start Level ' + nextLevelNumber,
					videoSrc: videoSrc
				});
			}, 1000);
		} else {
			setTimeout(function() {
				if (typeof showEndOverlay === 'function') showEndOverlay('win');
			}, 1000);
		}
	},
	shoot: function() {
